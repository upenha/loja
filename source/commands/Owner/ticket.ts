...
export class EvalCommand extends Command {
	...
	public async chatInputRun(interaction: Command.ChatInputInteraction) {
		...
		await interaction.channel?.send({ embeds: [new Embed().setImage('https://i.imgur.com/p112FgS.png').setTitle('`💸` | Deseja criar um ticket?').setDescription('Para criar um ticket clique no botão abaixo!')], components: [new MessageActionRow().addComponents(new MessageButton({ emoji: '📩', style: 'PRIMARY', label: 'Abrir ticket', customId: 'createTicket'}))]})
